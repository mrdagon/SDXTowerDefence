	...
	public:
		...
		void Shoot(IEnemy *対象)
		{
			...
			case UnitType::騎士://反射B-連射加速直進
				for (int a = 0; a < st->Hit数[Lv]; ++a)
				{
					IShot* shot = new S加速(&MEffect::弾, { { -速度 , 0.1, 速度*2 } }, DEF );
					SStage->Add(shot , a * 10);
				}
				break;
			case UnitType::斧士://吹き飛びA-後ろ射出加速
				SStage->Add(new S加速(&MEffect::弾, { { -速度 , 0.1 , 速度} }, DEF));
				break;
			case UnitType::闘士://吹き飛びB-
				break;
			case UnitType::武闘家://防御破壊A-加速弾-8WAY
				for (int a = 0; a < st->Hit数[Lv]; ++a)
				{
					角度 += PAI / 4;
					IShot* shot = new S加速(&MEffect::弾, { { 速度 * 2, -0.1, 速度 } }, DEF);
					SStage->Add(shot);
				}
				break;
